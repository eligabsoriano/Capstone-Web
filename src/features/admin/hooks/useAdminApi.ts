...
export const adminQueryKeys = {
    ...
    officerDetail: (id: string) => [...adminQueryKeys.officers(), "detail", id] as const,
    auditLogs: (filters?: { action?: string; limit?: number }) =>
...
export function useOfficersList(filters?: { active?: boolean; department?: string }) {
    return useQuery({
...
export function useUpdateOfficer(officerId: string) {
    ...
    return useMutation({
        ...
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: adminQueryKeys.officers() });
            queryClient.invalidateQueries({ queryKey: adminQueryKeys.officerDetail(officerId) });
        },
    });
}
